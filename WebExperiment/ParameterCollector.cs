...
namespace WebExperiment
{
    ...
    {
        public static ValueExtractor<TContext, T> CreateContext<TContext, T>(Transformer<TContext, T> parser) => (context, parameters, ct) =>
            parser(context);
        
        public static ValueExtractor<TContext, T> CreateSingle<TContext, T>(string name, Parser<T> parser) => (context, parameters, ct) =>
            parser(parameters[name]);

        public static ValueExtractor<TContext, T> CreateSingle<TContext, T>(string name, Parser<T> parser, T def) => (context, parameters, ct) =>
        {
            var str = (parameters.GetValues(name) ?? Enumerable.Empty<string>()).LastOrDefault();
            return str == null ? def : parser(str);
        };

        public static ValueExtractor<TContext, IEnumerable<T>> CreateArray<TContext, T>(string name, Parser<T> parser) => (context, parameters, ct) =>
    ...
    public class ParameterCollector<TContext, TResult, TController>
    {
        ...
        public ParameterCollector<TContext, CancellationToken, TResult, TController> CancellationToken() =>
            new ParameterCollector<TContext, CancellationToken, TResult, TController>(_controllerFactory) { Q1 = ParValExtBldr.CreateCancellationToken<TContext>()};
